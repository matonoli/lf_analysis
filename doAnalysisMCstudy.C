...
// running on the output histogram file instead of trees requires the file name to contain 'hist'
//
// RUNNING WITH ROOT6 FOR NOW REQUIRES TO RUN FIRST
// bash prep.sh XX Y     , where XX is the cut version and Y the wrapper type (1 trees vs. 2 ESD)
// alirooot ../compile.C
// .x doAnalysis(...)

void doAnalysisMCstudy(Long_t nEvents=10, const Char_t *flags = "0", const Char_t *inputFile="test.list", 
	...
	gSystem->Exec("cp ../MyKit/MyCascade.cxx .; cp ../MyKit/MyCascade.h .");
	gSystem->Exec("cp ../MyKit/Analyses/MyAnalysisMC.cxx .; cp ../MyKit/Analyses/MyAnalysisMC.h .");
	...
	if (fl.Contains("m")) root->LoadMacro("MyAnalysisMC.cxx+");
	...
	if (mode == trees) {
		...
		// OTHER ANALYSES
		if (fl.Contains("m")) {
			MyAnalysisMC* analysisMC					= new MyAnalysisMC();
			if (fl.Contains("m-")) analysisMC->SetRun(false);
			handler->AddAnalysis(analysisMC); }
		...
		root->ProcessLine("gObjectTable->Print()");
