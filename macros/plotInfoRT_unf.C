...
void plotInfoRT_unf() {
  ...
  //hNtvPtT = (TH2D*)dirCorr->Get(Form("hV0PtNtFitCorr_%s_RC_Trans",part.Data()));
  //hNtvPtN = (TH2D*)dirCorr->Get(Form("hV0PtNtFitCorr_%s_RC_Near",part.Data()));
  //hNtvPtA = (TH2D*)dirCorr->Get(Form("hV0PtNtFitCorr_%s_RC_Away",part.Data()));
  ...
  const double maxYratio = 5.1;

  const double minXleft = -0.5;
  const double maxXleft = 8.5;
...
//   A2->Draw();
  ...
  for (int iPt = 1; iPt < nPt+1; ++iPt)  {
    MakeNiceHistogram(hNtN[iPt], colors[0]);
    hNtN[iPt]->Draw("PLC PMC same");
  }
  ...
  hframeRatioLeft->Draw();
  for (int iPt = 1; iPt < nPt+1; ++iPt)  {
    MakeNiceHistogram(hNtA[iPt], colors[0]);
    hNtA[iPt]->Draw("PLC PMC same");
  }
  ...
  hframeRatioLeft->Draw();
  for (int iPt = 1; iPt < nPt+1; ++iPt)  {
    MakeNiceHistogram(hNtT[iPt], colors[0]);
    hNtT[iPt]->Draw("PLC PMC same");
  }
  latexR->DrawLatex(0.15,0.7,"Transverse, min.");


  C->cd(0);
  padTitleX->Draw();
  padTitleX->cd();
  const char* TitleX = "N_{T} min.";
  latexTitleX->DrawLatex(0.85, 0.6, TitleX);

   C->cd(0);
  padTitleY1->Draw();
  padTitleY1->cd();
  const char* TitleY1 = "P(N_{T} | p_{T})";
  latexTitleY1->SetTextAngle(90);
  latexTitleY1->DrawLatex(0.6, 0.3, TitleY1);
  ...
  C->SaveAs(Form("./InfoRT_%s_unf_min.pdf",part.Data()));
  C->SaveAs(Form("./InfoRT_%s_unf_min.png",part.Data()));
