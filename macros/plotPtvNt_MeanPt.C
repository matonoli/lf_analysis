...
void plotPtvNt_MeanPt() {
  ...
  //Bool_t isRC = true; Bool_t isRTMin = false; TString part("Lbar" ); TString partL("#Lambda");
  ...
  hNtvPtT = (TH2D*)dirCorr->Get(Form("hV0PtNtFitCorr_%s_D_TransMax",part.Data()));
  hNtvPtN = (TH2D*)dirCorr->Get(Form("hV0PtNtFitCorr_%s_D_Near",part.Data()));
  hNtvPtA = (TH2D*)dirCorr->Get(Form("hV0PtNtFitCorr_%s_D_Away",part.Data()));
  ...
  for (int iNt = 1; iNt < nNt+1; ++iNt) {
    ...
    meanPt = hPtT[iNt]->GetMean();
    meanPtErr = hPtT[iNt]->GetMeanError();
    ...
    meanPt = hPtN[iNt]->GetMean();
    meanPtErr = hPtN[iNt]->GetMeanError();
    ...
    meanPt = hPtA[iNt]->GetMean();
    meanPtErr = hPtA[iNt]->GetMeanError();
  ...
  const double minYratio = 0.75;
  const double maxYratio = 2.05;

  const double minXleft = -0.5;
  const double maxXleft = 39.0;
...
//   A2->Draw();
  ...
  latexTitleX->DrawLatex(0.9, 0.45, TitleX);

   C->cd(0);
  padTitleY1->Draw();
  padTitleY1->cd();
  const char* TitleY1 = "#LT p_{T} #GT";
  ...
  padTitleY2->Draw();
  padTitleY2->cd();
  const char* TitleY2 = "(GeV/#it{c})";
  latexTitleY2->SetTextAngle(90);
  latexTitleY2->DrawLatex(0.6, 0.05, TitleY2);

  C->SaveAs(Form("./PtvNt_MeanPt_%s.pdf",part.Data()));
  C->SaveAs(Form("./PtvNt_MeanPt_%s.png",part.Data()));
