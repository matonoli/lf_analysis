...
void plotPtvNt_Pt() {
  ...
  TFile* fIn = new TFile(Form("%s/outD_230123_0xcu_hist.root",path.Data()),"READ");
  ...
  hNtvPtT = (TH2D*)dirCorr->Get(Form("hV0PtNtMinFitCorrUnf_%s_D_TransMin",part.Data()));
  hNtvPtN = (TH2D*)dirCorr->Get(Form("hV0PtNtMinFitCorrUnf_%s_D_Near",part.Data()));
  hNtvPtA = (TH2D*)dirCorr->Get(Form("hV0PtNtMinFitCorrUnf_%s_D_Away",part.Data()));

  dirCorr = (TDirectoryFile*)fIn->Get("MyAnalysisV0correct_2");
  hPtDT = (TH1D*)((TH2D*)dirCorr->Get(Form("hV0PtNtMinFitCorr_%s_D_TransMin",part.Data())))->ProjectionX();
  hPtDN = (TH1D*)((TH2D*)dirCorr->Get(Form("hV0PtNtMinFitCorr_%s_D_Near",part.Data())))->ProjectionX();
  hPtDA = (TH1D*)((TH2D*)dirCorr->Get(Form("hV0PtNtMinFitCorr_%s_D_Away",part.Data())))->ProjectionX();
  ...
  //hPtDT = (TH1D*)hNtvPtT->ProjectionX();
  hNtDT = (TH1D*)hNtvPtT->ProjectionY();
  hPtDT->Scale(1./hPtDT->Integral());
  hNtDT->Scale(1./hNtDT->Integral());

  //hPtDN = (TH1D*)hNtvPtN->ProjectionX();
  hNtDN = (TH1D*)hNtvPtN->ProjectionY();
  hPtDN->Scale(1./hPtDN->Integral());
  hNtDN->Scale(1./hNtDN->Integral());

  //hPtDA = (TH1D*)hNtvPtA->ProjectionX();
  hNtDA = (TH1D*)hNtvPtA->ProjectionY();
  hPtDA->Scale(1./hPtDA->Integral());
  hNtDA->Scale(1./hNtDA->Integral());

  for (int iPt = 1; iPt < nPt+1; ++iPt) {
    hNtT[iPt] = (TH1D*)hNtvPtT->ProjectionY(Form("hNtT_%i",iPt),iPt,iPt);
    hNtT[iPt]->Scale( (hNtT[iPt]->Integral()>0) ? 1./hNtT[iPt]->Integral() : 1.);
    hNtT[iPt]->Divide(hNtDT);

    hNtN[iPt] = (TH1D*)hNtvPtN->ProjectionY(Form("hNtN_%i",iPt),iPt,iPt);
    hNtN[iPt]->Scale( (hNtN[iPt]->Integral()>0) ? 1./hNtN[iPt]->Integral() : 1.);
    hNtN[iPt]->Divide(hNtDN);

    hNtA[iPt] = (TH1D*)hNtvPtA->ProjectionY(Form("hNtA_%i",iPt),iPt,iPt);
    hNtA[iPt]->Scale( (hNtA[iPt]->Integral()>0) ? 1./hNtA[iPt]->Integral() : 1.);
    hNtA[iPt]->Divide(hNtDA);
  }

  for (int iNt = 1; iNt < nNt+1; ++iNt) {
    hPtT[iNt] = (TH1D*)hNtvPtT->ProjectionX(Form("hPtT_%i",iNt),iNt,iNt);
    hPtT[iNt]->Scale( (hPtT[iNt]->Integral()>0) ? 1./hPtT[iNt]->Integral() : 1.);
    hPtT[iNt]->Divide(hPtDT);

    hPtN[iNt] = (TH1D*)hNtvPtN->ProjectionX(Form("hPtN_%i",iNt),iNt,iNt);
    hPtN[iNt]->Scale( (hPtN[iNt]->Integral()>0) ? 1./hPtN[iNt]->Integral() : 1.);
    hPtN[iNt]->Divide(hPtDN);

    hPtA[iNt] = (TH1D*)hNtvPtA->ProjectionX(Form("hPtA_%i",iNt),iNt,iNt);
    hPtA[iNt]->Scale( (hPtA[iNt]->Integral()>0) ? 1./hPtA[iNt]->Integral() : 1.);
    hPtA[iNt]->Divide(hPtDA);
  }
  ...
  const double minYratio = 0.4;
  const double maxYratio = 2.6;
...
//   A2->Draw();
  ...
  nNt = 10; 
  for (int iNt = nNt; iNt > 0; --iNt)  {
    MakeNiceHistogram(hPtN[iNt], colors[0]);
    hPtN[iNt]->Draw("PLC PMC same");
  }

  latexSystem->DrawLatex(0.15,0.92,"pp, #sqrt{s} = 13 TeV");
  latexSystem->DrawLatex(0.15,0.86,"|#eta|<0.8");
  latexSystem->DrawLatex(0.15,0.80,"ALICE Data");
  ...
  hframeRatioLeft->Draw();
  for (int iNt = nNt; iNt > 0; --iNt)  {
    MakeNiceHistogram(hPtA[iNt], colors[0]);
    hPtA[iNt]->Draw("PLC PMC same");
  }
  latexSystem->DrawLatex(0.06,0.92,"(red) 0 < #it{N}_{T,min.} < 10 (blue)");
  ...
  hframeRatioLeft->Draw();
  for (int iNt = nNt; iNt > 0; --iNt)  {
    MakeNiceHistogram(hPtT[iNt], colors[0]);
    hPtT[iNt]->Draw("PLC PMC same");
  }
  ...
  padTitleX->Draw();
  padTitleX->cd();
  const char* TitleX = "p_{T} (GeV/#it{c})";
  latexTitleX->DrawLatex(0.80, 0.45, TitleX);

   C->cd(0);
  padTitleY1->Draw();
  padTitleY1->cd();
  const char* TitleY1 = "Ratio to";
  latexTitleY1->SetTextAngle(90);
  latexTitleY1->DrawLatex(0.6, 0.47, TitleY1);

  C->cd(0);
  padTitleY2->Draw();
  padTitleY2->cd();
  const char* TitleY2 = "N_{T} > 0";
  latexTitleY2->SetTextAngle(90);
  latexTitleY2->DrawLatex(0.6, 0.05, TitleY2);

  C->SaveAs(Form("./PtvNt_Pt_%s.pdf",part.Data()));
  C->SaveAs(Form("./PtvNt_Pt_%s.png",part.Data()));
