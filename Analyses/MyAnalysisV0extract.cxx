...
Bool_t MyAnalysisV0extract::BorrowHistograms() {
	...
	for (int iSp = 1; iSp < NSPECIES; ++iSp)	{
		...
		tV0mass[iSp][0][0] = (TNtuple*)mHandler->analysis(0)->dirFile()->Get(Form("tV0mass_%s_%s_%s",SPECIES[iSp],TYPE[0],MULTI[3]));
...
void MyAnalysisV0extract::ProduceRtSpectra() {
	...
	for (int iSp = 1; iSp < NSPECIES; ++iSp)	{
		...
		for (int iBin = 0; iBin < NRTBINS; ++iBin)	{
			...
			tV0mass[iSp][0][0]->SetName(Form("tree_iSp%i_iBin%i",iSp,iBin));
			
			yield = ExtractYieldFitRt((TTree*)tV0mass[iSp][0][0]->CopyTree(
				Form("lPt>%f && lPt<%f && lRt>%f && lRt<%f",cuts::RT_PTRANGE[0],cuts::RT_PTRANGE[1],
					RTBINS[iBin],RTBINS[iBin+1])));
		...
		//for entire rt range
		tV0mass[iSp][0][0]->SetName(Form("tree_iSp%i_iBin%i",iSp,NRTBINS));
			
		yield = ExtractYieldFitRt((TTree*)tV0mass[iSp][0][0]->CopyTree(
			Form("lPt>%f && lPt<%f && lRt>%f && lRt<%f",cuts::RT_PTRANGE[0],cuts::RT_PTRANGE[1],
				RTBINS[0],RTBINS[NRTBINS])));
		hRtV0Yields->SetBinContent(1+iSp,*(yield+0));
		hRtV0Yields->SetBinError(1+iSp,*(yield+1));
		//

		tV0mass[iSp][0][0]->SetName(treeName);
...
Double_t* MyAnalysisV0extract::ExtractYieldFitRt(TTree* tree) {
	...
	leg1->AddEntry((TObject*)0,Form("%4.2f < R_{T} < %4.2f (GeV/#it{c})",RTBINS[rtleft],RTBINS[rtright])," ");
	leg1->AddEntry((TObject*)0,Form("%s , #chi^{2}/ndf = %4.2f",SPECNAMES[spNumber],chi2ndf)," ");
	leg1->AddEntry((TObject*)0,Form("%4.1f #pm %4.1f",val[0],val[1])," ");
	leg1->Draw();
